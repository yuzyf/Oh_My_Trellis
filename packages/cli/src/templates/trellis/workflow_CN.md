<!-- 研究用中文对照读本 -->
<!-- 源文件: packages/cli/src/templates/trellis/workflow.md -->
...
- 1.3 配置上下文 `[required · once]` —— Claude Code、Cursor、OpenCode、Codex、Kiro、Gemini、Qoder、CodeBuddy、Copilot、Droid、Pi、Oh My Pi、ZCode、Reasonix（仅子代理分派平台；inline 平台跳过）
...
子代理分派协议适用于所有平台与所有子代理，包括 class-2 的 Codex/Gemini/Qoder/Copilot/ZCode/Reasonix/Trae 以及 `trellis-research`：每个分派提示都以 `Active task: <task path from task.py current>` 开头，然后再写角色专属说明。
